...
export default function BlockProfilePage() {
  ...
  const fetchBlockProfile = async () => {
    ...
    try {
      ...
      const blockResponse = await fetch(`/api/blocks/${params.id}`);
      if (blockResponse.ok) {
      ...
      const profileResponse = await fetch(`/api/blocks/${params.id}/profile`);
      ...
      toast.error("Failed to load block profile");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const response = await fetch(`/api/blocks/${params.id}/profile`, {
      ...
      toast.success("Block profile updated successfully");
      router.push(`/block/${params.id}`);
    } catch (error: any) {
      toast.error(error.message || "Failed to save block profile");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handlePhotoUpload = async () => {
    ...
    try {
      ...
      formData.append("folder", "block-photos");
  ...
  return (
    <div className="space-y-8">
      <Button
        ...
        onClick={() => router.push(`/block/${params.id}`)}
      >
      ...
      <div>
        <h1 className="text-3xl font-bold">{blockInfo?.name || 'Block'} Profile</h1>
