...
export default function OrganisationProfilePage() {
  ...
  const fetchOrganisationProfile = async () => {
    ...
    try {
      ...
      const profileResponse = await fetch(`/api/organisation-profile/${organisationId}`);
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const response = await fetch(`/api/organisation-profile/${organisationId}`, {
  ...
  const handleImageUpload = async (file: File, type: 'banner' | 'profile') => {
    ...
    try {
      ...
      formData.append("folder", "organisation-profiles");
  ...
  return (
    <div className="space-y-8">
      <Button
        ...
        onClick={() => router.push(`/organisation/${organisationId}`)}
      >
